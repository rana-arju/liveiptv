...
export default function FavoritesPage() {
  ...
  const favoriteChannels = channels.filter(c => favorites.isFavorite(c.id));
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-4">
        ...
        <p className="text-muted-foreground font-bold">Loading your favorites...</p>
      </div>
    );
  }
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-muted-foreground mt-2 text-base md:text-lg font-medium">
          Your handpicked selection of premium streams
        </p>
      </div>
      ...
      {favoriteChannels.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-96 gap-4 text-center">
          ...
          <p className="text-muted-foreground font-bold text-lg">No favorites yet</p>
        </div>
      ) : (
