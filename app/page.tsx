...
export default function Home() {
  ...
  const featuredChannel = useMemo(() => {
    ...
    return (
      channels.find((c) => c.category?.toLowerCase().includes("news")) ||
      channels[0]
    );
  }, [channels]);
  ...
  const handleChannelSelect = useCallback(
    (channel: Channel) => {
      ...
      router.push(
        `/?category=${encodeURIComponent(channel.category || "Uncategorized")}`,
      );
    },
  ...
  return (
    <>
      {!selectedCategory ? (
              ...
              <div className="space-y-2 md:space-y-4 pb-12 px-4 md:px-0">
                {filteredCategoryStats.map((category) => (
                  <CategoryRow
                    ...
                    channels={
                      searchQuery
                        ? (category as any).filteredChannels
                        : channels.filter(
                            (c) =>
                              (c.category || "Uncategorized") === category.name,
                          )
                    }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6 min-h-full px-0 md:p-6 max-w-[1600px] mx-auto">
          {/* Player Section */}
          <div className="lg:col-span-2 flex flex-col gap-3 md:gap-6 h-fit">
            <div className="md:rounded-2xl overflow-hidden shadow-2xl border-y md:border border-border bg-black w-full">
              <LivePlayer
            ...
            {/* Player Info */}
            {currentChannel && (
              <div className="space-y-3 bg-card p-4 md:p-6 md:rounded-3xl border-y md:border border-border shadow-sm">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 md:gap-4">
                  <div className="space-y-1.5 md:space-y-2">
                    <div className="flex items-center gap-2 md:gap-3">
                      <div className="px-2 py-0.5 bg-red-600 rounded text-[10px] font-black text-white uppercase tracking-widest animate-pulse">
                        LIVE
                      </div>
                      <h2 className="text-xl md:text-2xl font-black text-card-foreground tracking-tight">
                        {currentChannel.name}
                      </h2>
                    </div>
          ...
          <div className="flex flex-col gap-3 md:gap-4 lg:h-[calc(100vh-12rem)] min-h-[400px] overflow-hidden bg-card md:rounded-2xl border-y md:border border-border shadow-sm">
